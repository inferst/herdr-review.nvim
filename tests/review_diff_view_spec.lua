describe("review diff view", function()
  ...
  describe("review diff rendering internals", function()
    it("opens aligned unlisted old and new buffers", function()
      ...
      assert.is_false(vim.bo[view.left_buf].buflisted)
      assert.is_false(vim.bo[view.right_buf].buflisted)
      assert.are.equal("▼ M lua/example.lua  +1 -1", vim.api.nvim_buf_get_lines(view.left_buf, 0, 1, false)[1])
      assert.are.equal("two", vim.api.nvim_buf_get_lines(view.left_buf, 2, 3, false)[1])
      assert.are.equal("three", vim.api.nvim_buf_get_lines(view.right_buf, 2, 3, false)[1])
    end)
    ...
    it("extends line highlights across the screen line", function()
      ...
      local new_marks = vim.api.nvim_buf_get_extmarks(view.right_buf, render_ns, 0, -1, { details = true })
      ...
      local old_marks = vim.api.nvim_buf_get_extmarks(view.left_buf, render_ns, 0, -1, { details = true })
      ...
      local cursorline_marks = vim.api.nvim_buf_get_extmarks(view.right_buf, cursorline_ns, 0, -1, { details = true })
    ...
    it("keeps intra-line highlights visible over cursorline", function()
      ...
      local marks = vim.api.nvim_buf_get_extmarks(view.right_buf, render_ns, 0, -1, { details = true })
        ...
        files = {
          {
            left_path = "lua/example.lua",
            right_path = "lua/example.lua",
            left_text = "local value = 1",
            right_text = "local value = 2",
            status = "modified",
          },
        },
      })
    ...
    it("keeps the old side empty for an added file", function()
      ...
      assert.are.equal("▼ A —  +2 -0", vim.api.nvim_buf_get_lines(view.left_buf, 0, 1, false)[1])
      assert.are.equal("▼ A lua/added.lua  +2 -0", vim.api.nvim_buf_get_lines(view.right_buf, 0, 1, false)[1])
      local placeholder = vim.api.nvim_buf_get_lines(view.left_buf, 1, 2, false)[1]
      assert.are.equal("", placeholder)

      local marks = vim.api.nvim_buf_get_extmarks(view.left_buf, render_ns, 0, -1, { details = true })
      ...
      assert.are.equal("one", vim.api.nvim_buf_get_lines(view.right_buf, 1, 2, false)[1])
      assert.is_nil(view:resolve_location({ file = "lua/added.lua", side = "left", line = 1 }))
    end)
  end)
  ...
  it("maps the active cursor to a side-specific source location", function()
    ...
    assert.are.same({ file = "lua/example.lua", side = "right", line = 2, col = 0 }, view:get_cursor_location())
  end)
  ...
  it("exposes review metadata without mutable internals", function()
    ...
    metadata.files[1].right_path = "mutated.lua"
    assert.are.equal("lua/example.lua", view:metadata().files[1].right_path)
  end)
  ...
  it("returns cursor context with source context", function()
    ...
    assert.is_nil(err)
    assert.are.same({ file = "lua/example.lua", side = "right", line = 2, col = 0 }, cursor.location)
  ...
  it("resolves anchors through the public handle", function()
    ...
    assert.are.same({ file = "lua/example.lua", side = "right", line = 3 }, location)
  end)
  ...
  it("places annotations through source locations", function()
    ...
    local result = view:sync_annotations({
      {
        id = "comment-1",
        anchor = { file = "lua/example.lua", side = "right", line = 2 },
        text = "Review this",
      },
      {
        id = "comment-2",
        anchor = { file = "lua/example.lua", side = "right", line = 99 },
        text = "Stale",
      },
    })
  ...
  it("syncs annotations through anchors and reports moved and stale comments", function()
    ...
    assert.are.same({ "comment-1" }, result.applied)
    assert.are.same({ "comment-2" }, result.stale)
    assert.are.same({ file = "lua/example.lua", side = "right", line = 3 }, result.resolved["comment-1"])
    assert.are.same({
      location = { file = "lua/example.lua", side = "right", line = 3 },
      context = "one\ntwo\nchanged",
      context_start = 2,
  ...
  it("reanchors a location from its saved context", function()
    ...
    assert.are.same(
      { file = "lua/example.lua", side = "right", line = 3 },
      view:resolve_anchor({ file = "lua/example.lua", side = "right", line = 2, context = "one\ntwo\nchanged" })
    )
  end)
  ...
  it("moves by hunk instead of changed line", function()
    ...
    assert.are.same({ file = "lua/example.lua", side = "right", line = 5, col = 0 }, view:get_cursor_location())
  end)
  ...
  it("opens the target file when hunk navigation lands in a collapsed file", function()
    ...
    assert.are.same({ file = "lua/b.lua", side = "right", line = 2, col = 0 }, view:get_cursor_location())
  end)
  ...
  it("preserves the active diff side when moving between hunks", function()
    ...
    view:open_location({ file = "lua/example.lua", side = "left", line = 2 })

    view:move_hunk(1)

    assert.are.same({ file = "lua/example.lua", side = "left", line = 5, col = 0 }, view:get_cursor_location())
  end)
  ...
  it("captures layout and side-specific cursor state", function()
    ...
    assert.are.same({ file = "lua/a.lua", side = "left", line = 3, col = 0 }, snapshot.cursor)
  end)
  ...
  it("captures the last diff cursor after focus moves to the originating tab", function()
    ...
    assert.are.same({ file = "lua/a.lua", side = "right", line = 3, col = 0 }, snapshot.cursor)
  end)
  ...
  it("captures the path from the active side for renamed files", function()
    ...
    set_cursor_at_location(view, { file = "old.lua", side = "left", line = 1 })

    local snapshot = view:capture_state()

    assert.are.same({ file = "old.lua", side = "left", line = 1, col = 0 }, snapshot.cursor)
  end)
  ...
  it("restores the cursor column when replacing with the same review", function()
    ...
    local row = vim.api.nvim_win_get_cursor(view.right_win)[1]
    vim.api.nvim_win_set_cursor(view.right_win, { row, 2 })
    local snapshot = view:capture_state()

    assert.are.same({ file = "lua/a.lua", side = "right", line = 3, col = 2 }, snapshot.cursor)

    view:replace(state_input("review-state-col-restore"), { state = snapshot })

    vim.api.nvim_set_current_win(view.right_win)
    assert.are.same({ file = "lua/a.lua", side = "right", line = 3, col = 2 }, view:get_cursor_location())
  end)
